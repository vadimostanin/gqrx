...
bool RtlSdrSource::init()
{
    bool result = true;
    do
    {
        const char *devname = rtlsdr_get_device_name(m_dev_index);
        if (devname == NULL)
        {
            result = false;
        }
        ...
        int r = rtlsdr_open(&m_dev, m_dev_index);
        if (r < 0) {
...
vector<string> RtlSdrSource::get_device_names()
{
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        result.push_back(string(manufact) + string(" ") + string(rtlsdr_get_device_name(i)));
    }
